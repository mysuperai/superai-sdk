...
def memo(method, filename, folder=None, refresh=False):
    ...
    try:
        ...
        print("Executing memo of {}/{}...".format(settings.name, filename))
        ...
        if refresh:  # if forced refresh, then redo the method
            print("Refresh True {0}".format(method.__name__))
        ...
        if os.path.isfile(filepath):  # if have local cache, great, then continue
            ...
            try:  # try checking s3 for cache first, if exist, then return the value
                ...
                if e.response["Error"]["Code"] == "404":  # no local/s3 cache, produce the task, cache in local and s3
                    print("The S3 and local cache does not exist.")
                    return refresh_push_to_s3(method, filepath, client)
                else:
                    raise  # other s3 errors
    finally:
        print("Memo elapsed time: {} secs".format(time() - start_time))
...
def forget_memo(filename, folder=None, prefix: str = None):
    ...
    if filename:
        ...
        if os.path.isfile(filepath):
            print(f"Removing local memo for {folder}/{filename}")
            os.remove(filepath)
        else:
            print(f"No local memo found for {folder}/{filename}")

        try:
            print(f"Removing s3 memo for {s3_bucket}/{folder}/{filename}")
            session = boto3.session.Session()
            client = session.resource("s3")
    ...
    if prefix:
        try:
            ...
            if os.path.isdir(dirpath):
                print(f"Removing local memo for {dirpath}")
                shutil.rmtree(dirpath)
            else:
                print(f"No local memo found for {folder}/{filename}")
        except Exception as e:
            print(f"Exception while deleting memo {e}")

        try:
            s3_prefix = f"{folder}/{prefix}" if prefix.endswith("/") else f"{folder}/{prefix}/"
            print(f"Removing s3 memo for Bucket={s3_bucket} Prefix={s3_prefix}")
...
def delete_all_objects(Bucket, Prefix, MaxKeys=50, KeyMarker=None):
    ...
    try:
        ...
        print(f"Deleted Memo Bucket={Bucket}, Delete='Objects': {objects}")
