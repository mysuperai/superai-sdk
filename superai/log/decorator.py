...
class LoggingDecorator(object):
    ...
    def build_extensive_kwargs(fn, *args, **kwargs):
        ...
        extensive_kwargs.apply_defaults()
...
class log_on_end(LoggingDecorator):
    def __init__(self, log_level, message, *, logger=None, result_format_variable="result"):
...
class log_on_error(LoggingDecorator):
    def __init__(
        self, log_level, message, *, logger=None, on_exceptions=None, reraise=None, exception_format_variable="e"
    ):
        super().__init__(log_level, message, logger=logger)
        ...
        if reraise is None:
            warn(
                "The default value of the `reraise` parameter will be changed "
