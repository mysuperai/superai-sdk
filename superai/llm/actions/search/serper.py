...
class SerperSearchAPI(BaseModel):
    """Wrapper for Google Search API using Serper.dev.
    Get your API key at https://serper.dev/
    Add the API key to your environment as SERPER_API_KEY
    Add to configuration file as: serper_api_key
    """
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        serper_api_key = get_from_dict_or_env(values, "serper_api_key", "SERPER_API_KEY", default=config.serper_api_key)
    ...
    def _parse_results(self, results: dict) -> str:
        ...
        for result in results["organic"][: self.n_results]:
    ...
    def _google_serper_search_results(self, search_term: str, search_type: str = "search", **kwargs: Any) -> dict:
        headers = {
    ...
    async def _async_google_serper_search_results(
        ...
        if not self.aiosession:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, params=params, headers=headers, raise_for_status=False) as response:
                    search_results = await response.json()
        else:
            async with self.aiosession.post(url, params=params, headers=headers, raise_for_status=True) as response:
                search_results = await response.json()

        return search_results


class SerperSearchAction(BaseAction):
