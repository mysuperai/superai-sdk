...
by passing the named parameter :attr:`searx_host <SearxSearchAPI.searx_host>`
...
Then create a searx search instance like this:
    ...
    .. code-block:: python
        ...
        from langchain.utilities import SearxSearchAPI
        ...
        search = SearxSearchAPI(searx_host=searx_host)
...
Use the :meth:`run() <SearxSearchAPI.run>` and
:meth:`results() <SearxSearchAPI.results>` methods to query the searx API.
...
:py:class:`SearxSearchAPI` instance.

In the following example we are using the
:attr:`engines <SearxSearchAPI.engines>` and the ``language`` parameters:

    .. code-block:: python

        # assuming the searx host is set as above or exported as an env variable
        s = SearxSearchAPI(engines=['google', 'bing'],
                            language='es')
...
For example the following query:
    ...
    .. code-block:: python
        ...
        s = SearxSearchAPI("langchain library", engines=['github'])

        # can also be written as:
        s = SearxSearchAPI("langchain library !github")
        # or even:
        s = SearxSearchAPI("langchain library !gh")
...
class SearxSearchAPI(BaseModel):
    ...
    Example:
        .. code-block:: python
            ...
            from langchain.utilities import SearxSearchAPI
            searx = SearxSearchAPI(searx_host="http://localhost:8888")
    ...
    def validate_params(cls, values: Dict) -> Dict:
        ...
        searx_host = get_from_dict_or_env(values, "searx_host", "SEARX_HOST", default=config.searx_host)
        ...
        **kwargs: Any,
        ...
        Example:
            ...
            .. code-block:: python
                ...
                from langchain.utilities import SearxSearchAPI
                searx = SearxSearchAPI(searx_host="http://my.searx.host")
